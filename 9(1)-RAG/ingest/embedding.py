...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    def process_batch(batch_info):
        ...
        client_idx = start_idx // BATCH_SIZE % len(clients)
        client = clients[client_idx]
        vectors = _embed_batch_safe(client, batch_texts)
        for j, vec in enumerate(vectors):
            all_embeddings[start_idx + j] = vec
        with lock:
            completed[0] += 1
    ...
    if len(clients) > 1:
        ...
        for b in tqdm(batches, desc="Embedding"):
            process_batch(b)
